...
class MessagingManager:
    ...
    def add_direct(self, facade: DirectMessageFacade) -> None:
        ...
        if facade.exchange_name in [direct.exchange_name for direct in self._directs]:
            raise ValueError(
                "DirectMessageFacade with exchange "
                f"'{facade.exchange_name}' already exists."
            )
    ...
    def add_directs(self, facades: list[DirectMessageFacade]) -> None:
        ...
        Args:
            facades (list[DirectMessageFacade]):
                The list of DirectMessageFacade instances to add.

        Raises:
            ValueError:
                If any DirectMessageFacade with the same exchange already exists.
